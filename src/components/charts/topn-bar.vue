  ...
  export default {
    ...
    methods: {
      ...
      setChart() {
        let vm = this;
        let data = vm.data;
        let nameArr = [], maxArr = [], minArr = [], avgArr = [], countArr = [], formatNameArr = [];
        let maxLength = 60;
        ...
        if (_.isEmpty(data)) {
          ...
          vm.noData = false;
          data.map((m, i) => {
            let formatName = m.name.replace(/[\n\r]/g, ' ').replace(/\s+/g, ' ').substring(0, maxLength);
            formatName = m.name.length >= maxLength ? formatName + '...' : formatName;
            nameArr.push(m.name);
              ...
              formatter: function (params, ticket, callback) {
                ...
                params.map(m => {
                  ...
                  if (curItem) {
                    ...
                    let maxStr = max ? window.$$apm.locale['max'] + ': ' + max + 'ms<br/>' : '';
                    let minStr = min ? window.$$apm.locale['min'] + ': ' + min + 'ms<br/>' : '';
                    let avgStr = avg ? window.$$apm.locale['avg'] + ': ' + util.toDecimal(avg, 3) + 'ms<br/>' : '';
                    let countStr = count ? window.$$apm.locale['count'] + ': ' + count + '<br/>' : '';
            ...
            series: [
              {
                name: '平均',
                type: 'bar',
                barMaxWidth: formatNameArr.length <= 5 ? (formatNameArr.length * 20 + '%') : '100%',
                barMinHeight: 15,
                barCategoryGap: '30%',
                itemStyle: {
                  // emphasis:{
